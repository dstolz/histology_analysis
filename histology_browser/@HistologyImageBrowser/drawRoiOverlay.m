...
%
% Top right, because DRAWIMAGETILE writes the tile's own label along the top
% left inside the same box. Both are pinned to the top edge, where the eye goes
% first; putting them in opposite corners is what keeps a long section label
% from being overwritten by "UNSAVED EDITS", which is the one word on the tile
% that must never be hidden.

for iBadge = 1:numel(badges)
    style = styles{iBadge};

    text(ax, 0.98, 0.98 - 0.07 * (iBadge - 1), badges(iBadge), ...
        Units = "normalized", ...
