classdef SectionTracker < handle
    ...
    methods
        % Declared here because their bodies live in separate files in this
        % class folder, which is how MATLAB is told they are methods at all.
        % The private block below does the same for the three it holds.

        read(obj)                       % Read the tab into Table.

        report = ensureSchema(obj, options)  % Add the columns a write needs.

        T = metadataTable(obj, options) % The tab as COMBINE_VALUES_CSV takes it.

        idx = findRows(obj, criteria)   % Rows matching column and value pairs.

        report = updateRows(obj, criteria, updates, options)  % Write by criteria.

        report = updateByUid(obj, uids, updates, options)     % Write by Row UID.
